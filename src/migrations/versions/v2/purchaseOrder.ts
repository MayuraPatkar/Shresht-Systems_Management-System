...
export async function migratePurchaseOrders(db: any): Promise<{
    ...
    // 2. Migrate V1 purchases to V2 purchaseorders
    for (const doc of legacyPurchases) {
        try {
            ...
            if (!supplierId) {
                logger.warn(`No resolved supplier found for purchase ${doc.purchase_order_id} (Key: ${supplierKey})`);
            }
            ...
            const isReceived = !!doc.purchase_invoice_id;

            // Insert or Update the document in purchaseorders collection preserving the V1 MongoDB _id
            await purchaseordersCollection.updateOne(
                { _id: doc._id },
                {
                    $set: {
                        ...
                        purchase_invoice_no: doc.purchase_invoice_id || undefined,
                        purchase_date: doc.purchase_date ? new Date(doc.purchase_date) : new Date(),
                        due_date: doc.due_date ? new Date(doc.due_date) : undefined,
                        purchase_status: isReceived ? "Received" : "Ordered",
                        status: isReceived ? "Invoiced" : "Issued/Sent",
                        purchase_type: isInterState ? "Interstate" : "Local",
                        supplier_id: supplierId,
                        supplier_snapshot: supplierSnapshot,
                        items: mappedItems,
                        totals,
                        remarks: doc.remarks || `Migrated from purchase V1. Original ID: ${doc.purchase_order_id}`,
            ...
            logger.error(`Failed to migrate purchase record ${doc.purchase_order_id}:`, { error: msg });
        }
    }
