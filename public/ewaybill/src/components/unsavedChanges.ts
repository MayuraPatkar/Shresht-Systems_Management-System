/**
 * Unsaved Changes Protection for E-Way Bill Form
 * 
...
(function () {
    ...
    const collectFormData = () => (window as any).collectWayBillFormData ? (window as any).collectWayBillFormData() : null;
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-2xl max-w-md w-full mx-4 overflow-hidden unsaved-changes-panel">
                ...
                <div class="px-6 py-5">
                    <p class="text-sm text-slate-600 leading-relaxed">
                        You have unsaved changes in your e-way bill form. Would you like to save your progress as a draft, discard your changes, or continue editing?
                    </p>
                </div>
    ...
    async function handleSaveDraft() {
        ...
        try {
            ...
            const statusSelect = document.getElementById('ewaybill-status') as HTMLSelectElement | null;
            if (statusSelect) {
            ...
            if (!ewaybillApi) {
                throw new Error('ewaybillApi helper not found');
            }

            const response = await ewaybillApi.saveEWayBill(formData);

            if (response) {
                // Update ewaybill ID if returned (new e-way bill)
                if (response.ewaybill_id || response.id) {
                    const formEl = document.getElementById('waybill-form');
                    if (formEl) formEl.dataset.ewaybillId = response.ewaybill_id || response.id;
                ...
                if ((window as any).showToast) {
                    (window as any).showToast('E-Way Bill saved as draft.', 'success');
                }
    ...
    (window as any).markWayBillFormDirty = markDirty;
    (window as any).markWayBillFormClean = markClean;
    (window as any).isWayBillFormDirty = isDirty;
    (window as any).guardWayBillNavigation = guardNavigation;
