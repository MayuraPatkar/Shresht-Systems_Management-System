...
function addItem(insertAtIndex) {
  ...
  card.innerHTML = `
    ...
    <div class="item-field qty">
      <input type="number" placeholder="0" min="0.01" step="0.01" required class="qty-input">
    </div>
  ...
  row.innerHTML = `
    ...
    <td><input type="number" placeholder="Qty" min="0.01" step="0.01" required class="qty-input"></td>
  ...
  if (stockData) {
    if (isEWayBill) {
      ...
      if (tableRow) {
        ...
        const qtyInput = tableRow.querySelector(".qty-input") || tableRow.querySelector("input[placeholder='Qty']");
        ...
        if (rateInput) {
          rateInput.value = stockData.gst_rate ?? stockData.GST ?? 0;
      ...
      const qtyInput = element.querySelector(".qty-input") || element.querySelector("input[placeholder='Qty']");
