// @ts-nocheck
/**
 * Keyboard Shortcuts Utility for Customer Module
 * Handles all list-level, modal-level, and details-level navigation, action, and filter shortcuts.
 */

(function () {
    const isMac = navigator.userAgent.toLowerCase().includes('mac');
    const isDetailsPage = window.location.pathname.includes('/details') || !!document.getElementById('header-customer-name');
    ...
    const SHORTCUT_GROUPS = [
        ...
        {
            title: isDetailsPage ? 'Profile Actions' : 'Customer Actions',
    ...
    if (isDetailsPage) {
        SHORTCUT_GROUPS.push({
            ...
            items: [
                ...
                { label: 'Open Quotations Tab', keys: ['Alt', 'Q'] },
                { label: 'Open Invoices Tab', keys: ['Alt', 'I'] },
                { label: 'Open Services Tab', keys: ['Alt', 'S'] },
    ...
    function switchTab(direction: 'next' | 'prev') {
        const tabs = ['overview', 'quotations', 'invoices', 'services', 'payments'];
        ...
        if (targetBtn) {
            targetBtn.click();
            // Optional: Smooth scroll tab bar if overflowed
    ...
    function handleKeyboardShortcuts(event: KeyboardEvent) {
        ...
        if (event.key === 'Escape') {
            ...
            // Close customer form modal if open
            if (isModalOpen('customer-modal')) {
                event.preventDefault();
                event.stopPropagation();
                ...
                if (closeBtn) {
                    closeBtn.click();
                } else if ((window as any).customerForms?.closeModal) {
                    (window as any).customerForms.closeModal();
                }
                return;
            }
            ...
            // Close archive modal if open
            if (isModalOpen('archive-modal')) {
                event.preventDefault();
                event.stopPropagation();
                const closeArchiveBtn = document.getElementById('cancel-archive-btn');
                if (closeArchiveBtn) {
                    closeArchiveBtn.click();
                }
                return;
            }

            // If not typing, perform reset filters or go back to customer home
            if (!isTypingContext()) {
        ...
        // 4. Handle Save Customer Shortcut (Ctrl + S)
            ...
            switch (keyLower) {
                ...
                case 'e': {
                    ...
                    if (isDetailsPage) {
                        ...
                        // On list page, open the edit modal of the first customer if no modal is active
                        if (!isModalOpen('customer-modal')) {
                            const editCardBtn = document.querySelector('.customer-card-premium .edit-card-btn') as HTMLElement;
                            if (editCardBtn) {
                                event.preventDefault();
                                event.stopPropagation();
                                editCardBtn.click();
                ...
                case 'a': {
                    // View Archived Customers (Ctrl + Shift + A)
                    if (isShiftPressed && !isDetailsPage) {
                        const archivedBtn = document.getElementById('archived-customers-btn');
                        if (archivedBtn) {
                            event.preventDefault();
                            event.stopPropagation();
                            archivedBtn.click();
                ...
                case 'p': {
                    // Export Customer / Print (Ctrl + P) - prevent print page, show export modal instead in Profile
                    if (isDetailsPage) {
                ...
                case 'c': {
                    // Copy Customer ID (Ctrl + C) when not selecting text
        ...
        // 7. Alt key shortcuts (Alt + P, Alt + Q, Alt + S, Alt + O, Alt + I) for Tab Switching
        if (isAltPressed && isDetailsPage) {
            let targetTab = '';
            switch (keyLower) {
                case 'o': targetTab = 'overview'; break;
                case 'q': targetTab = 'quotations'; break;
                case 'i': targetTab = 'invoices'; break;
                case 's': targetTab = 'services'; break;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Register keydown listener in the capture phase for global command intercepting
        document.addEventListener('keydown', handleKeyboardShortcuts, true);
    });

    // Cleanup event handler mapping (in case we reload scripts dynamically)
    (window as any).cleanupCustomerShortcuts = () => {
