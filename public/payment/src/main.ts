...
(function (): void {
    ...
    async function loadCompanyInfoForForm() {
        ...
        try {
            const company = await (window as any).companyConfig.getCompanyInfo();
            if (company) {
                currentVoucherCompany = company;
                ...
                const phoneStr = company.phone ? `${company.phone.ph1 || ''}${company.phone.ph2 ? ' / ' + company.phone.ph2 : ''}` : '';
                
                if ($vPreviewCompanyName) $vPreviewCompanyName.textContent = (company.company_name || 'SHRESHT SYSTEMS').toUpperCase();
                if ($vPreviewCompanyAddress) $vPreviewCompanyAddress.textContent = addressStr || 'Company Address';
                if ($vPreviewCompanyContact) $vPreviewCompanyContact.textContent = `Ph: ${phoneStr} | Email: ${company.email || ''}`;
            }
        } catch (e) {
            console.error('Failed to load company info for voucher form:', e);
