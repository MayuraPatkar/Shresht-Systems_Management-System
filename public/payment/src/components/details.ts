...
(function (): void {
    ...
    function populateDetails(payment: IPaymentRecord) {
        ...
        const $downloadVoucherBtn = document.getElementById('details-download-voucher-btn');
        const $downloadReceiptBtn = document.getElementById('details-pdf-btn');
        const $sendReceiptBtn = document.getElementById('details-send-receipt-btn');
        ...
        if (vNo) {
            ...
            if ($downloadVoucherBtn) $downloadVoucherBtn.classList.remove('hidden');
            if ($downloadReceiptBtn) $downloadReceiptBtn.classList.add('hidden');
            if ($sendReceiptBtn) $sendReceiptBtn.classList.add('hidden');
        } else {
            $detailsVoucher.textContent = '-';
            if ($sendVoucherBtn) $sendVoucherBtn.classList.add('hidden');
            if ($downloadVoucherBtn) $downloadVoucherBtn.classList.add('hidden');
            if ($downloadReceiptBtn) $downloadReceiptBtn.classList.remove('hidden');
            if ($sendReceiptBtn) $sendReceiptBtn.classList.remove('hidden');
        }
    ...
    // Download Voucher button handler
    const $detailsDownloadVoucherBtn = document.getElementById('details-download-voucher-btn') as HTMLButtonElement | null;
    $detailsDownloadVoucherBtn?.addEventListener('click', async () => {
        if (!currentPayment || !currentPayment.voucher_no) return;

        if ($detailsDownloadVoucherBtn) {
            $detailsDownloadVoucherBtn.disabled = true;
            $detailsDownloadVoucherBtn.classList.add('opacity-70', 'cursor-not-allowed');
            $detailsDownloadVoucherBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Fetching...';
        }
        ...
        try {
            ...
            const htmlContent = generateVoucherPrintHTML(data.voucher, data.company);
            const cleanPartyName = (data.voucher.partyName || 'Voucher').replace(/[^a-zA-Z0-9-_]/g, '_');
            const filename = `PaymentVoucher-${cleanPartyName}-${data.voucher.voucherNumber}`;
            ...
            alert(err.message || 'Failed to download voucher.');
        } finally {
            if ($detailsDownloadVoucherBtn) {
                $detailsDownloadVoucherBtn.disabled = false;
                $detailsDownloadVoucherBtn.classList.remove('opacity-70', 'cursor-not-allowed');
                $detailsDownloadVoucherBtn.innerHTML = '<i class="fas fa-file-pdf text-red-650 text-xs"></i> Download Voucher';
            }
        }
    });
