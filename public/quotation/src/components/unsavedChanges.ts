/**
 * Unsaved Changes Protection for Quotation Form
 * 
...
(function () {
    ...
    const collectFormData = () => (window as any).collectQuotationFormData ? (window as any).collectQuotationFormData() : null;
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-2xl max-w-md w-full mx-4 overflow-hidden unsaved-changes-panel">
                ...
                <div class="px-6 py-5">
                    <p class="text-sm text-slate-600 leading-relaxed">
                        You have unsaved changes in your quotation form. Would you like to save your progress as a draft, discard your changes, or continue editing?
                    </p>
                </div>
    ...
    async function handleSaveDraft() {
        ...
        try {
            ...
            const statusSelect = document.getElementById('quotation-status') as HTMLSelectElement | null;
            if (statusSelect) {
            ...
            if (response) {
                ...
                if (response.quotation_id) {
                    ...
                    if (idInput) idInput.value = response.quotation_id;
                ...
                if ((window as any).showToast) {
                    (window as any).showToast('Quotation saved as draft.', 'success');
                }
    ...
    (window as any).markQuotationFormDirty = markDirty;
    (window as any).markQuotationFormClean = markClean;
    (window as any).isQuotationFormDirty = isDirty;
    (window as any).guardQuotationNavigation = guardNavigation;
