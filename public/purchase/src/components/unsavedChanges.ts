/**
 * Unsaved Changes Protection for Purchase Form
 * 
...
(function () {
    ...
    const collectFormData = () => (window as any).collectPurchaseFormData ? (window as any).collectPurchaseFormData() : null;
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-2xl max-w-md w-full mx-4 overflow-hidden unsaved-changes-panel">
                ...
                <div class="px-6 py-5">
                    <p class="text-sm text-slate-600 leading-relaxed">
                        You have unsaved changes in your purchase form. Would you like to save your progress as a draft, discard your changes, or continue editing?
                    </p>
                </div>
    ...
    async function handleSaveDraft() {
        ...
        try {
            ...
            // Save purchase directly through endpoint
            const response = (window as any).sendDocumentToServer
                ? await (window as any).sendDocumentToServer('/purchase/save-purchase', formData)
                : await fetch('/purchase/save-purchase', {
                    method: 'POST',
            ...
            if (response) {
                // Update purchase ID if returned (new purchase)
                if (response.purchase_no || response.id) {
                    const idInput = document.getElementById('id') as HTMLInputElement | null;
                    if (idInput) idInput.value = response.purchase_no || response.id;
                ...
                if ((window as any).showToast) {
                    (window as any).showToast('Purchase saved as draft.', 'success');
                } else if ((window as any).electronAPI?.showAlert2) {
                    (window as any).electronAPI.showAlert2('Purchase saved as draft.');
                }
    ...
    (window as any).markPurchaseFormDirty = markDirty;
    (window as any).markPurchaseFormClean = markClean;
    (window as any).isPurchaseFormDirty = isDirty;
    (window as any).guardPurchaseNavigation = guardNavigation;
