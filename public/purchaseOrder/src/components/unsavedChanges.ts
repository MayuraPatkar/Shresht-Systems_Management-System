/**
 * Unsaved Changes Protection for Purchase Order Form
 * 
...
(function () {
    ...
    const collectFormData = () => (window as any).collectPurchaseOrderFormData ? (window as any).collectPurchaseOrderFormData() : null;
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-2xl max-w-md w-full mx-4 overflow-hidden unsaved-changes-panel">
                ...
                <div class="px-6 py-5">
                    <p class="text-sm text-slate-600 leading-relaxed">
                        You have unsaved changes in your purchase order form. Would you like to save your progress as a draft, discard your changes, or continue editing?
                    </p>
                </div>
    ...
    async function handleSaveDraft() {
        ...
        try {
            ...
            const purchaseOrderApi = (window as any).purchaseOrderApi;
            if (!purchaseOrderApi) {
                throw new Error('purchaseOrderApi helper not found');
            }

            // In backend routes / models, status is "Draft" by default, so we can save it directly
            const response = await purchaseOrderApi.savePurchaseOrder(formData);

            if (response) {
                // Update purchase order ID if returned (new purchase order)
                if (response.purchase_order_no || response.id) {
                    const idInput = document.getElementById('id') as HTMLInputElement | null;
                    if (idInput) idInput.value = response.purchase_order_no || response.id;
                ...
                if ((window as any).showToast) {
                    (window as any).showToast('Purchase Order saved as draft.', 'success');
                } else if ((window as any).electronAPI?.showAlert2) {
                    (window as any).electronAPI.showAlert2('Purchase Order saved as draft.');
                }
    ...
    function handleDiscard() {
        ...
        // Reset the form
        const form = document.getElementById('purchase-order') as HTMLFormElement | null;
        if (form) form.reset();
    ...
    // Exported function to execute modal navigation programmatically if needed
    ...
    (window as any).markPurchaseOrderFormDirty = markDirty;
    (window as any).markPurchaseOrderFormClean = markClean;
    (window as any).isPurchaseOrderFormDirty = isDirty;
    (window as any).guardPurchaseOrderNavigation = guardNavigation;
