(() => {
    ...
    if (!scrollContainer) {
        console.error('Invoice view scroll container not found');
        return;
    }
    ...
    // Also run when the details page is rendered by viewInvoice (danger zone or status tracker could show up)
    const originalViewInvoice = (window as any).viewInvoice;
    if (typeof originalViewInvoice === 'function') {
        (window as any).viewInvoice = async function(...args: any[]) {
            const result = await originalViewInvoice.apply(this, args);
